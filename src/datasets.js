...
class Dataset {
  ...
  get validDates() {
    return (this._validDates ??= this.computeValidDates());
  }
  ...
  async fetchData(date, signal, kepler = null) {
    ...
    if (kepler) {
      url = `https://ws.kepler51.com/prod/moby/v1/${kepler.model}/${kepler.vaar}/${kepler.elev}/${kepler.reference_time}/${kepler.reference_time}/wmb?BBOX=-180,-90,180,90&metric=false&WIDTH=1440&HEIGHT=721&REQUEST=GetData&float_width=16&version=latest&encoding=gzip`;
    }
    console.log("url:", url);
  ...
  closestValidDate(date, condition = () => true) {
    ...
    switch (this.interval) {
      ...
      default:
        candidate = new ZonedDateTime(date, true).round(
          this.interval.roundTo
        ).date;
        break;
    }
...
export class GriddedDataset extends Dataset {
  ...
  bufferToData(buffer) {
    return {
      ...
      get(lonLat) {
        return singleArrayDataGet(this, lonLat);
      },
      originalUnit: this.originalUnit,
      ...this.dataProps,
    };
...
export class ParticleDataset extends Dataset {
  ...
  bufferToData(buffer) {
    return {
      ...
      get(lonLat) {
        return pairedArrayDataGet(this, lonLat);
      },
      ...this.dataProps,
    };
  }
